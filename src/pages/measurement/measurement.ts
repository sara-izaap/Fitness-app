  ...
  getMeasurement() {
    ...
    this.userservice.getMeasurements(this.user_id).then(res =>{
      ...
      if(this.response.length){
        this.mea_mmcm = this.response[0]['measurement']['mmcm'];
        this.mea_mmcm = this.response[0]['measurement']['inches'];
        this.weight_units = this.response[0]['weight_unit'];

        let weightavl:any = this.response[0]['weight'].split(".");
  ...
  getBodyfatList() {
    ...
    this.userservice.getBodyfatlists(this.user_id).then(res =>{
      ...
      if(this.bodyfatres.length){
        this.fat_mmcm = this.bodyfatres[0]['bodyfat']['mmcm'];
        this.fat_inches = this.bodyfatres[0]['bodyfat']['inches'];

        this.calculate['left']  = this.bodyfatres[0]['left_fat'];
        this.calculate['right'] = this.bodyfatres[0]['right_fat'];
        this.calculate['total'] = this.bodyfatres[0]['total_fat'];
      }
    })
    .catch(error => console.log(error));
  }
